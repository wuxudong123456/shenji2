...
| **U1-U3** | 上线（灰度开关 / 溯源抽样 / 压测） | ⬜ 待做 | U4 |
| P8-12 | 质量评测（黄金集 + 准确率/漏报/误报，需标注集） | ⬜ 独立 | — |

**T1-T8 + §0 溯源全绿**（8 验收场景全通过）：主链通+能溯源+门禁拦+可恢复+并发不互覆+数据/文件跨项目隔离+金额阈值无万倍误判+LLM 停机降级不白屏+大数据扫描限时分页不超时。analysis 面 403 属网关鉴权（上线依赖）。余 **U1-U3 上线动作**（灰度/抽样/压测）+ **P8-12 质量评测**（需标注集，独立排期）。**U4 检查单+回滚预案已完成**（§15，[RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) + [ROLLBACK_PLAN.md](ROLLBACK_PLAN.md)）。
...
**未闭环（上线阻塞）**：①U1 灰度开关待建；②U2 溯源抽样验收待做；③U3 压测待做；④网关鉴权（analysis/documents/suspicion 面 403）。RELEASE_CHECKLIST §6 已列为上线前必闭环项。

**下一步**：U1 灰度开关（前端 feature flag，需设计决策）/ U2 溯源抽样 / U3 压测（工具选型）+ P8-12 质量评测（需标注集）。
